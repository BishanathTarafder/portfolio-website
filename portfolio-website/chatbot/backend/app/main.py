...
if api_key:
    ...
    logging.info(f"Set OPENAI_API_KEY from environment variable")
    logging.info(f"API key present: {bool(api_key)}")
    logging.info(f"API key starts with 'sk-': {api_key.startswith('sk-')}")
    logging.info(f"API key length: {len(api_key)}")
else:
    logging.error("OPENAI_API_KEY not found in environment variables")
    logging.error(f"Current working directory: {os.getcwd()}")
    logging.error(f"Environment file exists: {os.path.exists('.env')}")
    logging.error("Please ensure .env file is in the correct location with OPENAI_API_KEY set")
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://localhost:3000"],  # Change from ["*"] to specific origin
...
try:
    ...
    logging.info(f"MODEL_NAME: {os.getenv('MODEL_NAME')}")
    
    llm_manager = LLMManager()
    logging.info("LLM Manager initialized successfully")
    
    # Test Gemini authentication directly
    auth_success = llm_manager.test_gemini_auth()
    if auth_success:
        logging.info("Gemini authentication test passed successfully")
    else:
        logging.error("Gemini authentication test failed")
    ...
    logging.error(f"Error initializing components: {type(e).__name__}: {e}")
    import traceback
...
if __name__ == "__main__":
    import uvicorn
    # Increase timeout settings for the server
    uvicorn.run(
        "app.main:app", 
        host="0.0.0.0", 
        port=8000, 
        reload=True,
        timeout_keep_alive=65,  # Increase keep-alive timeout (default is 5 seconds)
