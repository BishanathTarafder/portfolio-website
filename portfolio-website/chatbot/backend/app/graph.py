...
# Load data
def load_data():
    projects_json = load_json_data(get_data_path("projects.json"))
    # Handle nested 'projects' key in the JSON structure
    projects_data = projects_json.get('projects', [])
...
def create_chat_graph():
    ...
    def handle_projects(state: ChatState) -> ChatState:
        ...
        if not projects_data:
            ...
            for project in projects_data:
                ...
                # Check for both github_link and github keys
                github_url = project.get("github_link") or project.get("github", "")
                if github_url:
    ...
    def handle_general(state: ChatState) -> ChatState:
        ...
        try:
            ...
            # 1. Format the messages using the LangChain template
            chain_input = {"history": history, "input": last_input}
            formatted_messages = prompt.invoke(chain_input).to_messages()
            
            # 2. Convert to google-genai contents format
            contents = []
            ...
            print(f"Error in handle_general: {e}")
