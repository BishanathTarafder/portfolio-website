...
class PortfolioChatbot:
    ...
    def __init__(self, llm, rag):
        ...
        try:
            with open("../data/projects.json", "r") as f:
                ...
                # Handle nested 'projects' key in the JSON structure
                self.projects_data = projects_json.get('projects', [])
                logging.info(f"Loaded {len(self.projects_data)} projects from projects.json")
        except Exception as e:
            logging.error(f"Could not load projects data: {e}")
    ...
    def handle_projects(self, state: ChatState) -> ChatState:
        ...
        try:
            logging.info(f"DEBUG: Processing projects query: {query}")
            logging.info(f"DEBUG: Projects loaded: {bool(self.projects_data)}")
            logging.info(f"DEBUG: Project count: {len(self.projects_data) if self.projects_data else 0}")
            
            if self.projects_data:
                # Get project context from RAG system
                try:
                    context = self.rag.get_context(query)
                    logging.info(f"DEBUG: Successfully retrieved RAG context for projects query")
                except Exception as e:
                    logging.error(f"DEBUG: Error getting RAG context: {type(e).__name__}: {str(e)}")
                    logging.error(f"DEBUG: Traceback: {traceback.format_exc()}")
                    context = ""
                
                # Format project information
                project_info = "Here are some relevant projects:\n\n"
                for i, project in enumerate(self.projects_data[:3]):  # Limit to 3 projects for brevity
                    logging.info(f"DEBUG: Processing project {i+1}: {project.get('title', 'Unnamed Project')}")
                    project_info += f"- {project.get('title', 'Unnamed Project')}: {project.get('description', 'No description')}\n"
                ...
                logging.warning(f"DEBUG: No project data available for query: {query}")
            ...
            logging.error(f"DEBUG: Projects error: {type(e).__name__}: {str(e)}")
            logging.error(f"DEBUG: Traceback: {traceback.format_exc()}")
            return {**state, "response": "I'm sorry, I encountered an error while retrieving project information."}
