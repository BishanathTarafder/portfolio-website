...
export function Header() {
  ...
  useEffect(() => {
    ...
    // Hero animations complete at 0.7s + 0.8s = 1.5s (last animation start + duration)
    const heroAnimationCompleteDelay = 1600; // 1.5s + 100ms buffer
    
    // Set initial state for desktop navbar buttons
    ...
    if (desktopResumeBtn instanceof HTMLElement) {
      desktopResumeBtn.classList.add('opacity-0');
      desktopResumeBtn.style.transform = 'translateY(-20px)';
      
      setTimeout(() => {
        desktopResumeBtn.classList.remove('opacity-0');
        desktopResumeBtn.classList.add('fadeInUp');
        desktopResumeBtn.style.transform = '';
      }, heroAnimationCompleteDelay + 500); // Appear after other nav items
    }

    // Animate mobile menu items when mobile menu opens
    if (mobileMenuOpen) {
      const mobileNavLinks = document.querySelectorAll('.mobile-menu-container a:not(.btn-outline)');
      ...
      const mobileResumeBtn = document.querySelector('.mobile-menu-container .btn-outline');
      if (mobileResumeBtn instanceof HTMLElement) {
        // Set initial state
        mobileResumeBtn.classList.add('opacity-0');
        mobileResumeBtn.style.transform = 'translateY(10px)';
        
        setTimeout(() => {
          mobileResumeBtn.classList.remove('opacity-0');
          mobileResumeBtn.classList.add('fadeInUp');
          mobileResumeBtn.style.transform = '';
        }, 700); // Appear after other mobile menu items
      }
    }
  }, [mobileMenuOpen]);
    ...
    `}>
      <div className="w-full px-4 sm:px-6 md:px-8 lg:px-12 py-3 sm:py-4 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center">
          <div className={`nav-links ${navLinkStyles.navLinkContainer}`}>
            {navItems.map((item, index) => (
              <ScrollLink 
                  ...
                  className={navLinkStyles['navbar-link']}
      ...
      >
        ...
        <div 
          ...
          role="menu"
        >
          <div className="flex flex-col h-full justify-center items-center space-y-8 py-8 mobile-menu-container">
            {navItems.map((item, index) => (
              <ScrollLink 
                key={item.path}
                to={item.path.substring(1)} // Remove the # from the path
