...
export function FeaturedProjects() {
  return (
    <section className="w-full projects bg-black" id="projects">
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8 py-24">
        <div className="section-title mb-16">
          <h2 className="text-4xl font-bold mb-4 opacity-0">Featured Projects</h2>
          <p className="text-gray-400 text-lg opacity-0">Some of my recent work</p>
        </div>
        <div className="grid grid-cols-1 gap-32">
        {featuredProjects.map((project, index) => (
          <div 
            key={index}
            className={`project-card opacity-0 relative grid grid-cols-1 md:grid-cols-12 gap-8 items-center ${index % 2 === 0 ? '' : 'md:text-right'}`}
          >
            {/* Project Image (Left for even, Right for odd) */}
            <div className={`md:col-span-7 relative ${index % 2 === 0 ? 'md:order-1' : 'md:order-2'}`}>
              <div className="relative group">
                <div className="relative w-full h-[350px] md:h-[400px] overflow-hidden rounded-lg">
            ...
            <div className={`md:col-span-5 ${index % 2 === 0 ? 'md:order-2' : 'md:order-1'}`}>
              <div className={`flex flex-col ${index % 2 === 0 ? '' : 'md:items-end'}`}>
                <p className="font-mono text-AAsecondary mb-2 text-lg">Featured Project</p>
                <h3 className="text-3xl font-bold text-gray-200 mb-5">{project.title}</h3>
                
                <div className="bg-gray-800/80 p-7 rounded-lg shadow-xl mb-5 backdrop-blur-sm">
                  <p className="text-gray-400 text-lg">{project.description}</p>
                </div>
                
                <ul className={`flex flex-wrap gap-4 mb-7 text-base font-mono text-gray-400 ${index % 2 === 0 ? '' : 'md:justify-end'}`}>
